...
console.log('🚀 Full MySQL to PostgreSQL Import\n');
...
async function fullImport() {
  ...
  try {
    ...
    // Step 1: Update results table structure
    console.log('📋 Updating results table structure...');
    const updateSQL = fs.readFileSync('update-results-table.sql', 'utf8');
    await client.query(updateSQL);
    console.log('✅ Results table updated\n');
    
    // Step 2: Read MySQL dump
    ...
    // Step 5: Import data in correct order
    ...
    for (const tableName of importOrder) {
      ...
      for (const insert of tableInserts) {
        try {
          let pgSQL = insert.sql.replace(/`/g, '');
    ...
    console.log('3. Login with your existing credentials');
